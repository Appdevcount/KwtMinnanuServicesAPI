...
namespace ETradeAPI
{
    public class MobileDataBase
    {
                ...
                using (var sCon = new SqlConnection(ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString))
                {
                    using (var sCmd = new SqlCommand("etrade.usp_GetOrgReqDetailsForPrint", sCon))
                    {
                        ...
                        if (sDataResult != null && sDataResult.Tables.Count > 0)
                        {
                            ...
                            FileStream fsreader1 = new FileStream(System.Web.HttpContext.Current.Server.MapPath(sImage), FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite);
        ...
        internal static String DownloadOrgReqDocForEservice(OpenDocumentParams data)
        {
            ...
            if (rslt.status == "0")
            {
                rslt.Data = DownloadOrgReqDocumentDSForEservice(data.EserviceRequestid, data.DocumentId, data.hiderefprofile);
            }
            return JsonConvert.SerializeObject(rslt, Formatting.None); ;
        }

        private static DataSet DownloadOrgReqDocumentDSForEservice(String EserviceRequestid, String sOrgReqDocId, String hidreprofile)
        {
            DataSet Ds = new DataSet();
            try
            {
        ...
        internal static String ERequestPrintHtml(BrokerUpdateModel data)
        {
            ...
            // if (rslt.status == "0")
            {
                RD.data = ERequestPrintHtmlDS(data.RequestNumber);
            }
            return JsonConvert.SerializeObject(RD, Formatting.None);
        }

        private static String ERequestPrintHtmlDS(String RequestNUmber)
        {
            // OPTokenId = AES.DecryptToken(System.Web.HttpUtility.UrlDecode(OPTokenId));
            DataSet sDataResult = new DataSet("DS");
            String myString = "";
        ...
        internal static String VerifyOTP(String email, String mobile, String tokenId, String mUserid)
        {
            ...
            if (rslt.status == "0")
            {
                rslt.Data = VerifyOTPDS(rslt.mUserId, email, mobile);
            }
            return JsonConvert.SerializeObject(rslt, Formatting.None);
        }

        private static DataSet VerifyOTPDS(String mUserId, String emailCode, String mobileCode, String refType = "", String Mode = "")
        {
        ...
        internal static String SignUp(User userData)
        {
            ...
            DataSet Ds = new DataSet();
            /*commented as we have moved our validation to user reg screen
Code not required as it is handled in blur event of user reg
            if (userData.LegalEntity == "2") {
            ...
            try
            {
                using (var sCon = new SqlConnection(connectionStr))
                {
                    using (var sCmd = new SqlCommand("etrade.usp_MApp_NewSignupAction", sCon))
                    {
                        ...
                        // added for importer license number by azhar
       ...
       string BodyHtmlFile = "~/EmailTempt.htm")
        {
            ...
            string UserHostAddress = HttpContext.Current.Request.UserHostAddress;
            LogUserActivity LUA = new LogUserActivity { ActivityPerformed = NotifTypeLog + " EMAIL",
                IPAddress = UserHostAddress,
                LoginTime = DateTime.Now.ToString() , OtherAdditionalInfo= emailId 
            };
