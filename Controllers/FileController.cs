...
namespace ETradeAPI.Controllers
{
    ...
    public class FileController : ApiController
    {
        ...
        [Route("UpdateUserActivityDetails")]
        [HttpPost]
        public HttpResponseMessage UpdateUserActivityDetails([FromBody] LogUserActivity UserActivityData)
        {
            return new HttpResponseMessage()
            {
                Content = new StringContent(MobileDataBase.UpdateUserActivityDetails(UserActivityData), System.Text.Encoding.UTF8, "application/json")
            };
        }
        ...
        public HttpResponseMessage UploadSingleFile()
        {
            ...
            string LicenseNumber = "", IssuanceDate = "", ExpiryDate = "", LicenseType="";//added newly
            tokenId = Convert.ToString(System.Web.HttpContext.Current.Request.Form["tokenId"]);
            mUserid = Convert.ToString(System.Web.HttpContext.Current.Request.Form["mUserid"]);
            DocumentName = Convert.ToString(System.Web.HttpContext.Current.Request.Form["DocumentName"]);
            DocumentType = Convert.ToString(System.Web.HttpContext.Current.Request.Form["DocumentType"]);
...
//  sPath = Path.Combine("Etrade\\" + UploadedFrom, DateTime.Now.Year + "\\" + DateTime.Now.Month.ToString("00") + "\\" + DateTime.Now.Day.ToString("00") + '\\' + OrgReqId + '\\' + ID);// System.Web.Hosting.HostingEnvironment.MapPath("~/locker/");
        ...
        public bool UploadSingleReqFile(string mUserid,string ReqId,string ReqNum,HttpPostedFileBase File ,string DataProfileClass)
        {
            string login = ConfigurationManager.AppSettings["UploadAccessUserId"].ToString();
            string domain = ConfigurationManager.AppSettings["UploadAccessDomainName"].ToString();
            string password = Crypt.DecryptData(ConfigurationManager.AppSettings["UploadAccessPassword"].ToString());
            ...
            //tokenId = Convert.ToString(System.Web.HttpContext.Current.Request.Form["tokenId"]);
            // mUserid = Convert.ToString(System.Web.HttpContext.Current.Request.Form["mUserid"]);
            DocumentName = File.FileName;// Convert.ToString(System.Web.HttpContext.Current.Request.Form["DocumentName"]);
            DocumentType = Convert.ToString(System.Web.HttpContext.Current.Request.Form["DocumentType"]);
            //ImportLicenseDoc = Convert.ToBoolean(System.Web.HttpContext.Current.Request.Form["ImportLicenseDoc"]);
            string UploadedFrom = DataProfileClass;// Convert.ToString(System.Web.HttpContext.Current.Request.Form["UploadedFrom"]);
                ...
                //{
                    ...
                    if (ConfigurationManager.AppSettings["UploadAccessImpersinationRequired"].ToString() == "Y")
                    {
                        using (UserImpersonation user = new UserImpersonation(login, domain, password))
                        {
                            if (user.ImpersonateValidUser())
                            {
                                UploadReqFile( mUserid, File,ReqId,ReqNum, DocumentName, DocumentType, UploadedFrom);//, OrgReqId, OrgId, ImportLicenseDoc, LicenseNumber, IssuanceDate, ExpiryDate, LicenseType);
                            }
                            else
                            {
                                throw new UnauthorizedAccessException("Access failed while uploading.");
        ...
        private bool UploadReqFile(string mUserid, HttpPostedFileBase File, string ReqId, string ReqNum, string DocumentName, string DocumentType, string UploadedFrom)//, string OrgReqId, string OrgId, bool ImportLicenseDoc, string LicenseNumber, string IssuanceDate, string ExpiryDate, string LicenseType)
        {
            MobileDataBase m = new MobileDataBase();
            string ID = m.GetNewIntKey("ScanRequestUploadDocs");
            ...
            string ShareFolderPath1 = Path.Combine(UploadedFrom, DateTime.Now.Year + "\\" + DateTime.Now.Month.ToString("00") + "\\" + DateTime.Now.Day.ToString("00") + '\\' + ReqId + '\\' + ID);// System.Web.Hosting.HostingEnvironment.MapPath("~/locker/");


            sPath = Path.Combine(UploadedFrom, DateTime.Now.Year + "\\" + DateTime.Now.Month.ToString("00") + "\\" + DateTime.Now.Day.ToString("00") + '\\' + ReqId + '\\' + ID);// System.Web.Hosting.HostingEnvironment.MapPath("~/locker/");
                                                                                                                                                                                    //  sPath = Path.Combine("Etrade\\" + UploadedFrom, DateTime.Now.Year + "\\" + DateTime.Now.Month.ToString("00") + "\\" + DateTime.Now.Day.ToString("00") + '\\' + OrgReqId + '\\' + ID);// System.Web.Hosting.HostingEnvironment.MapPath("~/locker/");
            ...
            Frdata.DocumentName = DocumentName;
            Frdata.DocumentType = DocumentType;
            Frdata.OrgReqId = ReqId;
            Frdata.UploadedFrom = UploadedFrom;// Convert.ToString(System.Web.HttpContext.Current.Request.Form["UploadedFrom"]);
            Frdata.FilePath = DateTime.Now.Year + "\\" + DateTime.Now.Month.ToString("00") + "\\" + DateTime.Now.Day.ToString("00") + '\\' + ReqId;
            Frdata.IsUploaded = 'n';

            if (ReqId != null)
            {
                Frdata.EserviceRequestId = ReqId;

                if (!Convert.ToString(ReqId).All(char.IsDigit))
                {
                    Frdata.EserviceRequestId = CommonFunctions.CsUploadDecrypt(Convert.ToString(ReqId).ToString());
                }
            }
            // CHECK THE FILE COUNT.
            FileInfo F = null;
            ...
            {
                //rslt.Data = 
                    MobileDataBase.InsertDROrgUserReqUploadData( mUserid, Frdata);
                return true;
        ...
        public HttpResponseMessage GetDocumentTypes([FromBody] OrgReqResultInfoParams data)
        {
            return new HttpResponseMessage()
            {
                Content = new StringContent(MobileDataBase.GetTypes("etrade.Usp_MApp_GetOrgDocTypes", data.lang, data.sOrgReqId, data.sOrgId), System.Text.Encoding.UTF8, "application/json")
            };
        }
        [Route("OrgDocTypes")]
        [HttpPost]
        public HttpResponseMessage GetOrgDocumentTypes([FromBody] OrgReqResultInfoParams data)
        {
            return new HttpResponseMessage()
            {
                Content = new StringContent(MobileDataBase.GetTypes("[ETRADE].[GetOrganizationDocTypes]", data.lang, data.sOrgReqId,data.sOrgId), System.Text.Encoding.UTF8, "application/json")
            };
        }
        [Route("GetOrganizationDocuments")]
        [HttpPost]
        public HttpResponseMessage GetOrganizationDocuments([FromBody] GetOrganizationDocuments data)
        {
            return new HttpResponseMessage()
            {
                Content = new StringContent(MobileDataBase.GetOrganizationDocuments(data.mUserid, data.OrganizationId, data.OrganizationRequestId), System.Text.Encoding.UTF8, "application/json")
